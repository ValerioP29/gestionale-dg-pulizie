<div
    x-data="addressAutocomplete({
        setAddress(value) { $wire.set('{{ $getStatePath('address') }}', value) },
        setLat(value)     { $wire.set('{{ $getStatePath('latitude') }}', value) },
        setLng(value)     { $wire.set('{{ $getStatePath('longitude') }}', value) },
    })"
    class="space-y-2"
>
...
@once
    ...
    <script>
        function addressAutocomplete(callbacks) {
            return {
                init() {
                    ...
                    autocomplete.addListener('place_changed', () => {
                        ...
                        callbacks.setLat(lat);
                        callbacks.setLng(lng);
                    });
                }
            }
        }
